...
func main() {
	...
	r.GET("/stream/single/:node_id", func(c *gin.Context) {
		nodeID := c.Param("node_id")
		envelopeFormat := c.DefaultQuery("envelope_format", "json") // get from query, default to json
		...
		for s := 1; s <= currentSeq; s++ {
			sendDataOverWS(state, conn, nodeID, s, envelopeFormat)
		}
		...
		for {
			...
			sendDataOverWS(state, conn, nodeID, seq, envelopeFormat)
		}
	})
...
func sendDataOverWS(state *AppState, conn *websocket.Conn, nodeID string, seqNum int, envelopeFormat string) {
	dataKey := fmt.Sprintf("data.%s.%d", nodeID, seqNum)
	...
	// Try to decode payload as float64 array (like Python)
	var payload interface{}
	if value.Payload != "" {
		raw, err := hex.DecodeString(value.Payload)
		if err == nil {
			// Try float64 array
			if len(raw)%8 == 0 {
				floats := make([]float64, len(raw)/8)
				for i := 0; i < len(floats); i++ {
					floats[i] = math.Float64frombits(binary.LittleEndian.Uint64(raw[i*8 : (i+1)*8]))
				}
				payload = floats
			} else {
				// Try JSON
				...
				if err := json.Unmarshal(raw, &obj); err == nil {
					...
					payload = value.Payload // fallback to hex string
				}
			}
		} else {
			payload = value.Payload // fallback to hex string
	...
	if envelopeFormat == "msgpack" {
		packed, _ := msgpack.Marshal(resp)
		conn.WriteMessage(websocket.BinaryMessage, packed)
	} else {
		conn.WriteJSON(resp)
	}
}
